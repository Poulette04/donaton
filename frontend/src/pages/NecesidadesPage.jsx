...
const NecesidadesPage = () => {
  ...
  const cambiarEstado = async (id, nuevoEstado) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/necesidades/${id}/estado?estado=${nuevoEstado}`, {
        method: 'PATCH',
        headers: token ? { Authorization: `Bearer ${token}` } : {}
      });
  ...
  const eliminarNecesidad = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/necesidades/${id}`, {
        method: 'DELETE',
        headers: token ? { Authorization: `Bearer ${token}` } : {}
      });
      fetchNecesidades();
