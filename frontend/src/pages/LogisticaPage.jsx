...
const LogisticaPage = () => {
  ...
  const fetchLogistica = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/logistica`);
  ...
  const fetchSedes = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/sedes`);
      const data = await response.json();
      const content = data.content || data;
      setSedes(content);
    } catch (error) {
  ...
  const fetchInventario = async () => {
    setLoadingInv(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/inventario`);
      const data = await response.json();
      const content = data.content || data;
  ...
  const fetchNecesidades = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/necesidades`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editandoId) {
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/logistica/${editandoId}`, {
          method: 'PUT',
        ...
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/logistica`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const eliminarEnvio = async (envio) => {
    ...
    if (window.confirm(msg)) {
      try {
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/logistica/${envio.id}`, { method: 'DELETE' });
        fetchLogistica();
        fetchInventario();
  ...
  const marcarEntregado = async (id) => {
    if (window.confirm("¿Marcar este envío como ENTREGADO? Se descontará del inventario automáticamente.")) {
      try {
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/logistica/${id}/estado`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
