...
const InventarioPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/inventario/${editingId}`, {
          method: 'PUT',
          headers: { 
        ...
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/inventario`, {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
            ...(token ? { Authorization: `Bearer ${token}` } : {})
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/inventario/${id}`, {
        method: 'DELETE',
